...
function validateUpdates(updates) {
  ...
  if (!hasUpdates) {
    return { valid: false, error: { code: 'NO_UPDATES', message: 'At least one of model, thinking, or verbose must be provided' } };
  }
  
  if (thinking !== undefined && !VALID_THINKING_VALUES.includes(thinking)) {
    return { valid: false, error: { code: 'INVALID_THINKING', message: `thinking must be one of: ${VALID_THINKING_VALUES.join(', ')}` } };
  }
  
  if (verbose !== undefined && typeof verbose !== 'boolean') {
    return { valid: false, error: { code: 'INVALID_VERBOSE', message: 'verbose must be a boolean' } };
  }
...
async function handleSessionsPatch(ws, msg) {
  ...
  // Validate sessionKey
  if (!sessionKey) {
    ws.send(JSON.stringify({ type: 'error', error: { code: 'INVALID_SESSION_KEY', message: 'sessionKey is required' } }));
    return;
  }
  
  if (!isCanonicalSessionKey(sessionKey)) {
    ws.send(JSON.stringify({ type: 'error', error: { code: 'INVALID_SESSION_KEY', message: 'Invalid sessionKey format' } }));
    return;
  }
  ...
  if (!parsed) {
    ws.send(JSON.stringify({ type: 'error', error: { code: 'INVALID_SESSION_KEY', message: 'Could not parse sessionKey' } }));
    return;
  }
  ...
  try {
    ...
    if (!fs.existsSync(sessionsFilePath)) {
      ws.send(JSON.stringify({ type: 'error', error: { code: 'SESSION_NOT_FOUND', message: 'Session file not found' } }));
      return;
    }
    ...
    try {
      ...
      ws.send(JSON.stringify({ type: 'error', error: { code: 'INTERNAL_ERROR', message: 'Failed to read or parse sessions file' } }));
      return;
    }
    
    // Check if session exists
    if (!sessions[storedKey]) {
      ws.send(JSON.stringify({ type: 'error', error: { code: 'SESSION_NOT_FOUND', message: 'Session not found in sessions file' } }));
      return;
    }
    ...
    try {
      ...
      ws.send(JSON.stringify({ type: 'error', error: { code: 'INTERNAL_ERROR', message: 'Failed to write sessions file' } }));
      return;
    }
    ...
    ws.send(JSON.stringify({ type: 'error', error: { code: 'INTERNAL_ERROR', message: err.message || 'Unknown error' } }));
  }
}
