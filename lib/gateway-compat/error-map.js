const TIMEOUT_CODES = Object.freeze(
  new Set([
    "ETIMEDOUT",
    "ESOCKETTIMEDOUT",
    "TIMEOUT",
...
const FORBIDDEN_CODES = Object.freeze(
  new Set(["FORBIDDEN", "UNAUTHORIZED", "PERMISSION_DENIED", "AUTH_FAILED"]),
);
...
function classifyGatewayError(err, _options = {}) {
  ...
  if (
    ...
    rawMessage &&
    /missing scope|unauthorized role/.test(messageLower)
  ) {
    return {
      code: "FORBIDDEN",
      status: 403,
  ...
  if (
    rawCode === "INVALID_REQUEST" &&
    /unknown cron job id|unknown session|session not found|invalid session key|not found/.test(
      messageLower,
    )
  ) {
    return {
