...
function normalizeCronPatchFromLegacy(body = {}) {
  ...
  const patch = {};
  const legacyKeys = ["name", "enabled", "agentId", "schedule", "sessionTarget", "wakeMode", "delivery", "payload"];
  for (const key of legacyKeys) {
    ...
    if (key === "name" || key === "schedule" || key === "sessionTarget" || key === "wakeMode" || key === "delivery") {
...
function normalizeCronListPayload(payload) {
  ...
  return {
    ...
    meta: Object.assign({ source: "gateway" }, isObject(sourcePayload.meta) ? sourcePayload.meta : {}),
  };
}
