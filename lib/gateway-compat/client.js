...
function resolveGatewayWsUrl(gatewayConfig = {}) {
  ...
  const port =
    Number.isFinite(Number(gatewayConfig.port)) && Number(gatewayConfig.port) > 0
...
function createGatewayClient({
  ...
  const scopes = normalizeScopes(connectScopes || resolveMethodScopes(method, methodScopeMap));
