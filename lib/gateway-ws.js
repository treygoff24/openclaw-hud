...
class GatewayWS extends EventEmitter {
  constructor({ url, token, reconnect = {}, requestTimeoutMs = 60000, connect = {} }) {
    ...
    this._connectRole = connect.role || 'operator';
    this._connectScopes = Array.isArray(connect.scopes) ? connect.scopes : ['operator.read', 'operator.write'];
    this._connectClient = {
      id: 'openclaw-ios',
      ...
      ws.on('message', (raw) => {
        ...
        if (msg?.type === 'event' && msg.event === 'connect.challenge' && !handshakeDone) {
          try {
            ...
            const error = err instanceof Error ? err : new Error(String(err || 'Handshake failed'));
            handshakeDone = true;
