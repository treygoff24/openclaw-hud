...
const { randomUUID, createPublicKey, createPrivateKey, createHash, sign } = require('crypto');
const WebSocket = require('ws');

const EVENT_MAP = { chat: 'chat-event', tick: 'tick', agent: 'agent-event', shutdown: 'shutdown' };
const SCOPES = ['operator.read', 'operator.write'];

function rawPublicKeyBase64Url(pem) {
...
class GatewayWS extends EventEmitter {
  constructor({ url, token, deviceIdentity, reconnect = {}, requestTimeoutMs = 60000 }) {
    super();
    this._url = url;
    this._token = token;
    this._identity = deviceIdentity || null;
  ...
  _buildConnectParams(nonce) {
    ...
    const params = {
      ...
      client: { id: 'gateway-client', version: '0.1.0', platform: 'hud', mode: 'backend' },
      auth: { token: this._token },
      role: 'operator',
      scopes: SCOPES,
    };
    if (this._identity) {
      const payload = ['v2', this._identity.deviceId, 'gateway-client', 'backend', 'operator',
        SCOPES.join(','), String(signedAt), this._token || '', nonce].join('|');
      params.device = {
        id: this._identity.deviceId,
        publicKey: rawPublicKeyBase64Url(this._identity.publicKeyPem),
        signature: signPayload(this._identity.privateKeyPem, payload),
        signedAt,
        nonce,
      };
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.on('message', (raw) => {
        ...
        if (msg.event === 'connect.challenge' && !handshakeDone) {
          ...
          ws.send(JSON.stringify({ type: 'req', id, method: 'connect', params: this._buildConnectParams(nonce) }));
          return;
        }
