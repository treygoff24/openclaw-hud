...
function buildRequestId(req) {
  return (
    clampText(req.get?.("x-request-id"), 64) || clampText(req.get?.("x-correlation-id"), 64) || null
  );
}
...
function createDefaultOptions(options = {}) {
  return {
    sampleRate: parseSampleRate(
      typeof options.sampleRate === "undefined"
        ? process.env.HUD_API_TAIL_TELEMETRY_SAMPLE_RATE
        : options.sampleRate,
    ),
...
function createApiTailTelemetryMiddleware(options = {}) {
  ...
  return function apiTailTelemetryMiddleware(req, res, next) {
    ...
    const onComplete = async () => {
      ...
      const cacheState = (res.locals?.apiTailTelemetry?.cacheState &&
        parseCacheState(res.locals.apiTailTelemetry.cacheState)) ||
      ...
      ensureApiTailEventSummary(
        ...
        "apiTail.cache",
        "cacheHit",
        cacheState.state === "hit" ? 1 : 0,
      );
      ensureApiTailEventSummary(
        event,
        "apiTail.cache",
        "cacheMiss",
        cacheState.state === "miss" ? 1 : 0,
      );
      ensureApiTailEventSummary(
        event,
        "apiTail.cache",
        "cacheStale",
        cacheState.state === "stale" ? 1 : 0,
      );
      ensureApiTailEventSummary(
        event,
        "apiTail.cache",
