...
(function() {
  ...
  function requireApi(obj, objName, methodName) {
    if (!obj || typeof obj[methodName] !== 'function') {
      throw new Error('[ChatWsHandler] Missing ' + objName + '.' + methodName + '. Check script load order.');
    }
  }
  ...
  window.ChatWsHandler = {
    ...
    handle: function(data) {
      ...
      switch (data.type) {
        case 'subscribed':
          return runtime.showLive(s.currentSession && data.sessionId === s.currentSession.sessionId);
        case 'chat-subscribe-ack':
          return runtime.showLive(s.currentSession && data.sessionKey === s.currentSession.sessionKey);
