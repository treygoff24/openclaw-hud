...
(function () {
  ...
  function createCopyTurnButton(assistantMsg) {
    var btn = window.CopyUtils.createCopyButton(
      function () {
        return buildTurnMarkdown(assistantMsg);
      },
      "Copy entire turn",
      { visibleLabel: "Copy turn" },
    );
  ...
  function renderHistoryMessage(msg) {
    ...
    if (role === "system") {
      ...
      blocks = Array.isArray(msg.content)
        ? msg.content
    ...
    if (role === "assistant") {
      var sender = resolveSenderDisplay(
        window.ChatState && (window.ChatState.currentSession || window.ChatState),
      );
    ...
    if (toolUseEls.length >= 3 && window.ChatToolBlocks) {
      ...
      toolUseEls.forEach(function (el) {
        div.appendChild(el);
      });
    }
  ...
  function createAssistantStreamEl() {
    ...
    var streamSession = resolveSenderDisplay(
      window.ChatState && (window.ChatState.currentSession || window.ChatState),
    );
    roleSpan.textContent =
      streamSession && streamSession.displayName ? streamSession.displayName : "assistant";
