...
function makeLiveWeeklyPayload({
  model,
  inputTokens,
  outputTokens,
  cacheReadTokens,
...
test.describe("Model usage (live-weekly contract)", () => {
  test("renders model usage panel from /api/model-usage/live-weekly with model/ tokens / cost fields", async ({
    page,
  }) => {
  ...
  test("updates rendered live-weekly values on a forced refresh cycle", async ({ page }) => {
    ...
    await expect
      .poll(
        ...
        { timeout: 5000, intervals: [100, 100, 200, 250, 500, 1000] },
      )
