...
function writeLocalHistoryFixture({ sessionKey, sessionId, lines, storeCanonicalKey = false }) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ [key]: { sessionId } }, null, 2),
  );
...
describe("chat-handlers", () => {
  describe("handleChatMessage", () => {
    it("chat-subscribe adds to maps and sends ack", async () => {
      ...
      expect(ws.send).toHaveBeenCalledWith(
        JSON.stringify({ type: "chat-subscribe-ack", sessionKey: MAIN_KEY }),
      );
      ...
      writeLocalHistoryFixture({
        ...
        lines: [
          JSON.stringify({
            type: "message",
            role: "assistant",
            content: "forbidden fallback works",
          }),
        ],
      });
      const ws = mockWs();
      ...
      writeLocalHistoryFixture({
        ...
        lines: [
          JSON.stringify({
            type: "message",
            role: "assistant",
            content: "unavailable fallback works",
          }),
        ],
      });
      const ws = mockWs();
    ...
    it("chat-history falls back when gateway times out", async () => {
      writeLocalHistoryFixture({
        ...
        lines: [
          JSON.stringify({ type: "message", role: "assistant", content: "timeout fallback works" }),
        ],
      });
      const ws = mockWs();
      const gw = mockGateway(true);
    ...
    it("chat-history logs correlation id and fallback reason fields", async () => {
      writeLocalHistoryFixture({
        ...
        lines: [
          JSON.stringify({ type: "message", role: "assistant", content: "log fallback works" }),
        ],
      });
      ...
      const fallbackSuccessLog = logs.find((line) =>
        line.includes("[CHAT-HISTORY] fallback-success"),
      );
    ...
    it("chat-history local fallback with limit parses from tail", async () => {
      ...
      try {
        await mod.handleChatMessage(
          ws,
          { type: "chat-history", sessionKey: MAIN_KEY, limit: 1 },
          gw,
        );
      } finally {
    ...
    it("chat-abort includes runId when provided", async () => {
      ...
      await mod.handleChatMessage(
        ws,
        { type: "chat-abort", sessionKey: MAIN_KEY, runId: "r1" },
        gw,
      );
    ...
    it("chat-new success returns spawned session key", async () => {
      ...
      await mod.handleChatMessage(
        ws,
        { type: "chat-new", model: "gpt-5", agentId: "agent1" },
        null,
      );
    ...
    it("chat-send without sessionKey sends error", async () => {
      ...
      await mod.handleChatMessage(
        ws,
        { type: "chat-send", message: "hi", idempotencyKey: "ik1" },
        gw,
      );
      const sent = JSON.parse(ws.send.mock.calls[0][0]);
      expect(sent.ok).toBe(false);
    ...
    it("chat-send without message sends error", async () => {
      ...
      await mod.handleChatMessage(
        ws,
        { type: "chat-send", sessionKey: MAIN_KEY, message: "", idempotencyKey: "ik1" },
        gw,
      );
      const sent = JSON.parse(ws.send.mock.calls[0][0]);
      expect(sent.ok).toBe(false);
  ...
  describe("setupChatEventRouting", () => {
    ...
    it("fans out chat-event to all subscribed ready clients only", async () => {
      ...
      await mod.handleChatMessage(
        wsOtherSession,
        { type: "chat-subscribe", sessionKey: OTHER_KEY },
        null,
      );
