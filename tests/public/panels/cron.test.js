...
describe('cron.render', () => {
  ...
  it('renders cron job count', () => {
    HUD.cron.render({ jobs: [
      { id: 'j1', name: 'Test Job', enabled: true, schedule: { expr: '*/5 * * * *', tz: 'UTC' }, state: {} }
    ]});
  ...
  it('renders job rows with correct structure', () => {
    HUD.cron.render({ jobs: [{
      id: 'j1', name: 'My Job', enabled: true, agentId: 'bot1',
      schedule: { expr: '0 * * * *', tz: 'America/Chicago' },
      state: { lastStatus: 'completed', lastRunAtMs: Date.now() - 60000, nextRunAtMs: Date.now() + 60000 }
    }]});
  ...
  it('shows error styling for failed jobs', () => {
    HUD.cron.render({ jobs: [{
      id: 'j1', name: 'Failed', enabled: true,
      schedule: { expr: '* * * * *' },
...
describe('cron.openEditor', () => {
  it('populates modal fields from job data', () => {
    HUD.cron.render({ jobs: [{
      id: 'j1', name: 'Edit Me', enabled: true, agentId: 'bot1',
