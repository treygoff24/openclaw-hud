...
describe("openChatPane", () => {
  ...
  it("sends chat-subscribe and chat-history over WS", () => {
    ...
    expect(
      calls.find((c) => c.type === "chat-subscribe" && c.sessionKey === "agent:agent1:sess1"),
    ).toBeTruthy();
    expect(
      calls.find((c) => c.type === "chat-history" && c.sessionKey === "agent:agent1:sess1"),
    ).toBeTruthy();
  });
  ...
  it("uses canonical session key when provided and resolves loading on matching history", () => {
    ...
    const calls = ws.send.mock.calls.map((c) => JSON.parse(c[0]));
    expect(
      calls.find((c) => c.type === "chat-subscribe" && c.sessionKey === "agent:agent1:main"),
    ).toBeTruthy();
    expect(
      calls.find((c) => c.type === "chat-history" && c.sessionKey === "agent:agent1:main"),
    ).toBeTruthy();
  ...
  it("enriches chat state with sender metadata from _allSessions", () => {
    ...
    window.openChatPane(
      "agent1",
      "my-sub-session",
      "AGENT:SUB:research-planner",
      "agent:agent1:my-sub-session",
    );
    ...
    expect(
      calls.find(
        (c) => c.type === "chat-subscribe" && c.sessionKey === "agent:agent1:my-sub-session",
      ),
    ).toBeTruthy();
  });
  ...
  it("uses backend sessionRole metadata for main sessions without spawnDepth", () => {
    ...
    const assistantMessage = window.ChatMessage.renderHistoryMessage({
      role: "assistant",
      content: "hello",
    });
  ...
  it("unsubscribes from previous session", () => {
    ...
    const calls = ws.send.mock.calls.map((c) => JSON.parse(c[0]));
    expect(
      calls.find((c) => c.type === "chat-unsubscribe" && c.sessionKey === "agent:a:s1"),
    ).toBeTruthy();
  });
...
describe("handleChatWsMessage — chat-history-result", () => {
  ...
  it("renders tool_result blocks as collapsed", () => {
    ...
    window.handleChatWsMessage({
      ...
      sessionKey: "agent:a:s",
      messages: [
        { role: "tool", content: [{ type: "tool_result", tool_use_id: "t1", content: "done" }] },
      ],
    });
    const tb = document.querySelector(".chat-tool-block");
    expect(tb).not.toBeNull();
...
describe("handleChatWsMessage — chat-event deltas", () => {
  ...
  it("shows stop button when run is active", () => {
    ...
    window.handleChatWsMessage({
      type: "chat-event",
      payload: { sessionKey: "agent:a:s", runId: "r1", state: "delta", seq: 1, message: "Hi" },
    });
  ...
  it("handles final state", () => {
    ...
    window.handleChatWsMessage({
      type: "chat-event",
      payload: { sessionKey: "agent:a:s", runId: "r1", state: "final", seq: 2 },
    });
  ...
  it("handles aborted state with badge", () => {
    ...
    window.handleChatWsMessage({
      type: "chat-event",
      payload: { sessionKey: "agent:a:s", runId: "r1", state: "delta", seq: 1, message: "partial" },
    });
    window.handleChatWsMessage({
      type: "chat-event",
      payload: {
...
describe("handleChatWsMessage — chat-send-ack", () => {
  ...
  it("removes pending class on success ack", () => {
    ...
    const key = JSON.parse(
      ws.send.mock.calls.find((c) => JSON.parse(c[0]).type === "chat-send")[0],
    ).idempotencyKey;
...
describe("handleChatWsMessage — gateway-status", () => {
  ...
  it("does not request duplicate history when reconnecting after history has loaded", () => {
    ...
    const calls = ws.send.mock.calls.map((c) => JSON.parse(c[0]));
    expect(
      calls.filter((c) => c.type === "chat-history" && c.sessionKey === "agent:a:s").length,
    ).toBe(0);
    expect(
      calls.filter((c) => c.type === "chat-subscribe" && c.sessionKey === "agent:a:s").length,
    ).toBe(1);
  });
});
...
describe("message input", () => {
  ...
  it("Enter sends message, Shift+Enter does not", () => {
    ...
    expect(
      ws.send.mock.calls.map((c) => JSON.parse(c[0])).filter((c) => c.type === "chat-send").length,
    ).toBe(0);
    input.dispatchEvent(
...
describe("multiple concurrent runs", () => {
  ...
  it("tracks multiple runs independently", () => {
    ...
    window.handleChatWsMessage({
      type: "chat-event",
      payload: { sessionKey: "agent:a:s", runId: "r1", state: "delta", seq: 1, message: "Run1" },
    });
    window.handleChatWsMessage({
      type: "chat-event",
      payload: { sessionKey: "agent:a:s", runId: "r2", state: "delta", seq: 1, message: "Run2" },
    });
...
describe("ChatState message cache", () => {
  ...
  it("evicts oldest messages when cap is exceeded", () => {
    ...
    expect(
      window.ChatState.currentMessages[window.ChatState.currentMessages.length - 1].content,
    ).toBe("msg-504");
  });
