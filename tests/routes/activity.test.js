...
describe('GET /api/activity', () => {
  ...
  it('truncates content to 200 characters', async () => {
    ...
    helpers.safeRead.mockReturnValue(JSON.stringify({ type: 'msg', timestamp: '2025-01-01T00:00:00Z', content: longContent }));
    const res = await request(createApp()).get('/api/activity');
  ...
  it('limits results to 50 events', async () => {
    ...
    const sessions = {};
    for (let i = 0; i < 3; i++) sessions[`s${i}`] = { sessionId: `s${i}`, updatedAt: Date.now() - i * 1000 };
    helpers.safeJSON.mockReturnValue(sessions);
    const lines = Array.from({ length: 5 }, (_, i) =>
      JSON.stringify({ type: 'msg', timestamp: `2025-01-01T${String(i).padStart(2, '0')}:00:00Z`, content: `e${i}` })
    ).join('\n');
  ...
  it('includes agentId and sessionKey in each event', async () => {
    ...
    helpers.safeJSON.mockReturnValue({ 'session-key': { sessionId: 'sid', updatedAt: Date.now() } });
    helpers.safeRead.mockReturnValue(JSON.stringify({ type: 'tool', name: 'read', timestamp: '2025-06-01T00:00:00Z' }));
    const res = await request(createApp()).get('/api/activity');
  ...
  it('uses name or type as content for non-string content', async () => {
    helpers.safeReaddir.mockReturnValue(['a1']);
    helpers.safeJSON.mockReturnValue({ 's1': { sessionId: 's1', updatedAt: Date.now() } });
    helpers.safeRead.mockReturnValue(JSON.stringify({ type: 'tool_use', name: 'exec', timestamp: '2025-01-01T00:00:00Z', content: { result: true } }));
    const res = await request(createApp()).get('/api/activity');
