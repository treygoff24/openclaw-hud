...
describe("usage archive", () => {
  ...
  it("fails when trying to overwrite an existing week snapshot", async () => {
    ...
    expect(() => writeWeeklySnapshot(snapshot, { openclawHome: TMP_ROOT })).toThrow(
  ...
  it("reads history in newest-first order and ignores malformed files", async () => {
    const { writeWeeklySnapshot, readWeeklyHistory, getUsageArchiveDir } =
      await import("../../lib/usage-archive.js");
